...
$configuracao = GraficoMagic::getData($model->id, $view, $tipo_grafico, $campo_id, FALSE);
...
$variaveis = $this->render("/_graficos/js/vars/common/uni");
    ...
    function create{$tipo_grafico}(_data)
    {
        ...
        function filtrar(param) 
        {
            ...
            if(!_last)
            {
                ...
                if (navigator.userAgent.indexOf("Firefox") > 0 && '{$view}' != 'content') 
                {
                    _dataFiltro = _dataFiltro.reverse();
                }
            
                var _category = _dataFiltro[param.dataIndex];
