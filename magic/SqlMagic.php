...
SQL;
    ...
    public static function getData($consulta, $index = 0, $filtro = null, $token = '', $previous = FALSE, $limit = 10, $userChoices = FALSE, $user_id = null, $print = FALSE, $painel = null) {
        ...
        foreach ($argumentos as $i => $argumento) {
            ...
            if ($i == $index) {
                ...
                $tipo_grafico = ($userChoices) ? ActiveGraphMagic::getActiveGraph($elemento_atual, $user_id) : $elemento_atual->tipo_grafico;

                if ($elemento_atual->campo->link) {
                    $links['x'] = $elemento_atual->campo->link;
                }

            } elseif ($i == ($index - 1) && !$previous && !$print) {
                //aqui não entra
                $data_filtro[$nome_campo] = $filtro;
                $data_breadcrumb[$index - 1] = ['nome' => $argumento->campo->nome, 'valor' => $filtro];
            } elseif ($i == ($index - 1) && $painel) {
//                var_dump($painel, ('valor' . ($argumento->campo->ordem - 1)));
            }
        }
        ...
        $configuracao_filtro = ($userChoices) ? ConsultaFiltroUsuario::find()->andWhere([
                    'id_usuario' => $user_id,
        ...
        $where = self::formatWhere($condicao, $data_filtro, $condicao_usuario, $consulta->condicao_avancada);

        $sql = self::montaSql($consulta, $select, $where, $group_by, $order_by, $limit);
    ...
    public static function formatWhere($condicao, $drilldown, $condicao_usuario = null, $condicao_avancada = null) {
        if (!$condicao && !$drilldown && !$condicao_usuario && (!$condicao_avancada || trim($condicao_avancada) == '')) {
            return '';
        }
    ...
    public static function getTotalUserChanges($consulta_id, $user_id = null) {
        if (!$user_id) {
            $user_id = Yii::$app->user->id;
        }
        ...
        if($user_id) {
            ...
            SELECT 
                sel.quantidade AS quantidade
            FROM
            (
                SELECT 
                    'G' AS quantidade
                FROM
                    bpbi_consulta_grafico_usuario
                WHERE
                    is_ativo = 1 AND is_excluido = 0
                    AND id_usuario = {$user_id}
                    AND id_consulta = {$consulta_id}
                    AND campo IN 
                    (
                        SELECT 
                            campo.campo
                        FROM
                            bpbi_consulta_item item
                        JOIN bpbi_indicador_campo campo ON campo.id = item.id_campo
                        WHERE
                            id_consulta = {$consulta_id}
                            AND parametro = 'argumento'
                            AND item.is_ativo = 1
                            AND item.is_excluido = 0
                            AND campo.is_ativo = 1
                            AND campo.is_excluido = 0
                    )
                UNION ALL 
                SELECT 
                    'F' AS quantidade
                FROM
                    bpbi_consulta_filtro_usuario
                WHERE
                    id_usuario = {$user_id}
                    AND id_consulta = {$consulta_id}
                    AND condicao != '[]'
...
SQL;
    ...
    public static function getTotalChanges($consulta_id) {
